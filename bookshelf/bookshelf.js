...
async function deleteSeriesBox(){if(!editingSeriesId||!confirm('BOXを削除しますか？ 本そのものは削除されません。'))return;try{await authorRequest(`/author/series/${editingSeriesId}`,{method:'DELETE'});writeIdList(SERIES_BOX_ORDER_KEY,readIdList(SERIES_BOX_ORDER_KEY).filter(id=>id!==editingSeriesId));await loadAuthorShelfData();$('#seriesBoxDialog').close();toast('シリーズBOXを削除しました。');}catch(error){$('#seriesBoxStatus').textContent=error.message;}}
