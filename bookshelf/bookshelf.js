...
const STUDIO_DRAFT_DB_VERSION=2;
...
async function deleteStudioDraft(draftId){if(!draftId)return;const db=await openStudioDraftDb();try{const transaction=db.transaction([STUDIO_DRAFT_STORE,STUDIO_DRAFT_META_STORE],'readwrite');await Promise.all([STUDIO_DRAFT_STORE,STUDIO_DRAFT_META_STORE].map(name=>request(transaction.objectStore(name).delete(draftId))));}finally{db.close();}}
