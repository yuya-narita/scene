...
function publicAuthorShelfUrl(authorId){const url=new URL('../author/',location.href);url.searchParams.set('id',String(authorId||''));return url.toString();}
...
function renderAuthorHeaderPreviews(){const header=signedInAuthor?.bookshelfHeader,url=authorHeaderPublicUrl(header),desktop=$('#authorHeaderDesktopPreview'),mobile=$('#authorHeaderMobilePreview');if(!desktop||!mobile)return;const apply=(node,x,y)=>{node.style.backgroundImage=url?`url("${url.replace(/"/g,'\\"')}")`:'';node.style.backgroundPosition=`${x}% ${y}%`;const empty=node.querySelector('em');if(empty)empty.hidden=!!url;};apply(desktop,headerPositionValue('#authorHeaderDesktopX',header?.desktopX??50),headerPositionValue('#authorHeaderDesktopY',header?.desktopY??50));apply(mobile,headerPositionValue('#authorHeaderMobileX',header?.mobileX??50),headerPositionValue('#authorHeaderMobileY',header?.mobileY??50));}
function syncAuthorHeaderUI(){const header=signedInAuthor?.bookshelfHeader,controls=$('#authorHeaderPositionControls');if(!controls)return;controls.hidden=!header;for(const [id,key] of [['#authorHeaderDesktopX','desktopX'],['#authorHeaderDesktopY','desktopY'],['#authorHeaderMobileX','mobileX'],['#authorHeaderMobileY','mobileY']]){const input=$(id);if(input)input.value=String(header?.[key]??50);}renderAuthorHeaderPreviews();}
...
async function saveAuthorHeaderPosition(){if(!signedInAuthor?.bookshelfHeader)return;setAuthorHeaderStatus('表示位置を保存しています…');try{const body={desktopX:headerPositionValue('#authorHeaderDesktopX'),desktopY:headerPositionValue('#authorHeaderDesktopY'),mobileX:headerPositionValue('#authorHeaderMobileX'),mobileY:headerPositionValue('#authorHeaderMobileY')};const payload=await authorRequest('/author/bookshelf/header',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});saveAuthorSession(authorToken,payload.author);syncAuthorHeaderUI();setAuthorHeaderStatus('PC・iPhoneの表示位置を保存しました。');}catch(error){setAuthorHeaderStatus(error.message||'位置を保存できませんでした。',true);}}
...
$('#copyPublicAuthorShelf').onclick=async()=>{if(!signedInAuthor?.authorId)return;try{await navigator.clipboard.writeText(publicAuthorShelfUrl(signedInAuthor.authorId));toast('公開本棚のURLをコピーしました。');}catch(_){toast('URLをコピーできませんでした。');}};
...
for(const id of ['#authorHeaderDesktopX','#authorHeaderDesktopY','#authorHeaderMobileX','#authorHeaderMobileY'])$(id)?.addEventListener('input',()=>{renderAuthorHeaderPreviews();setAuthorHeaderStatus('プレビューを確認して「位置を保存」を押してください。');});
