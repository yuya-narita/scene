...
function syncAuthorProfileUI(){const name=$('#bookshelfAuthorProfileName'),lead=$('#bookshelfAuthorShelfLead');if(name)name.value=signedInAuthor?.displayName||'';if(lead)lead.value=signedInAuthor?.bookshelfLead||'';}
...
async function saveAuthorProfile(){const displayName=$('#bookshelfAuthorProfileName')?.value.trim()||'',bookshelfLead=$('#bookshelfAuthorShelfLead')?.value.trim()||'';if(!displayName){setAuthorProfileStatus('作者名を入力してください。',true);return;}setAuthorProfileStatus('作者情報を保存しています…');try{const payload=await authorRequest('/author-auth/profile',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({displayName,bookshelfLead})});saveAuthorSession(authorToken,payload.author);setAuthorProfileStatus('公開作者情報を保存しました。');}catch(error){setAuthorProfileStatus(error.message||'作者情報を保存できませんでした。',true);}}
