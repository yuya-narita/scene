...
function detailWorkInfoHtml(w){
  const subtitle=String(w?.subtitle||'').trim(),description=String(w?.description||'').trim(),seriesTitle=String(w?.seriesTitle||'').trim(),episode=String(w?.episode||'').trim(),episodeTitle=String(w?.episodeTitle||'').trim();
  if(!subtitle&&!description&&!seriesTitle&&!episode&&!episodeTitle)return'';
  const seriesBits=[seriesTitle,episode].filter(Boolean).join(' · ');
  return`<section class="detail-work-info"><p class="eyebrow">WORK INFO</p>${seriesBits?`<p class="detail-series">${escapeHtml(seriesBits)}</p>`:''}${episodeTitle?`<h3>${escapeHtml(episodeTitle)}</h3>`:''}${subtitle?`<p class="detail-subtitle">${escapeHtml(subtitle)}</p>`:''}${description?`<p class="detail-description">${escapeHtml(description)}</p>`:''}</section>`;
}
