...
async function restoreServerPublishedMasters(){
  ...
  for(const work of authorWorks){
    ...
    if(!workId||!publicationId||localIds.has(workId))continue;
    try{
      const res=await fetch(`${API_BASE}/work/${encodeURIComponent(publicationId)}?raw=1`,{cache:'no-store'});
      if(!res.ok)continue;
      const blob=await res.blob();
      const file=new File([blob],`${String(work?.title||workId).replace(/[\/:*?"<>|]+/g,'_')}.scene`,{type:'application/octet-stream'});
      const info=await inspectMaster(file);
      if(String(info.workId)!==workId)continue;
      await addMaster(file,{silent:true});
      localIds.add(workId);restored++;
    }catch(error){console.warn('Published Master restore skipped',workId,error);}
  }
  if(restored){
    const [masters,drafts]=await Promise.all([getAllWorks(),getAllStudioDraftMeta()]);
    const draftWorkIds=new Set(drafts.map(item=>String(item?.workId||'')).filter(Boolean));
    const localMasters=await Promise.all(masters.map(async item=>hydrateStoredBookMetadata({...item,role:'master'})));
    const draftViews=drafts.filter(item=>!draftWorkIds.has('')&&!localMasters.some(master=>master.workId&&master.workId===item.workId)).map(item=>({...item,role:'studio-draft'}));
