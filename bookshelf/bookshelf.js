...
async function openDetail(role,id){
  ...
  if(role==='studio-draft'){
    ...
    if(isPublished){
      ...
      $('#exportMaster').onclick=async()=>{try{const publicationId=String(publishedWork?.publicationId||publishedWork?.publicId||'').trim();if(!publicationId)throw new Error('PUBLICATION_ID_MISSING');const res=await fetch(`${API_BASE}/work/${encodeURIComponent(publicationId)}?raw=1`,{cache:'no-store'});if(!res.ok)throw new Error(`HTTP ${res.status}`);downloadBlob(await res.blob(),`${w.title||'master'}.scene`);}catch(error){console.error(error);toast('Masterを書き出せませんでした。');}};
