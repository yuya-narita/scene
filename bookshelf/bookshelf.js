...
const WORK_RECOVERY='workRecovery';
const READER_RECOVERY='readerRecovery';
const STUDIO_DRAFT_DB_NAME='scene-studio-drafts';
const STUDIO_DRAFT_DB_VERSION=3;
const STUDIO_DRAFT_STORE='drafts';
const STUDIO_DRAFT_META_STORE='draftMeta';
...
const SERIES_BOX_ORDER_KEY='ahako:bookshelf:series-box-order';
const OWNED_SERIES_BOXES_KEY='ahako:bookshelf:owned-series-boxes:v1';
const OWNED_SERIES_BOX_ORDER_KEY='ahako:bookshelf:owned-series-box-order:v1';
...
let authorSeries=[];
let authorWorks=[];
let ownedSeriesBoxes=[];
let editingSeriesId='';
let editingSeriesWorkIds=[];
let editingSeriesShelf='created';
let draggingSeriesBoxId='';
let suppressSeriesBoxClick=false;
let draggingSeriesWorkId='';

function authorHeaders(extra={}){return {...extra,...(authorToken?{Authorization:`Bearer ${authorToken}`}:{})};}
function publicAuthorShelfUrl(author=signedInAuthor){const data=typeof author==='string'?{authorId:author}:author||{},url=new URL('../author/',location.href);if(data.slug)url.searchParams.set('u',data.slug);else url.searchParams.set('id',String(data.authorId||''));return url.toString();}
...
async function saveAuthorProfile(){const displayName=$('#bookshelfAuthorProfileName')?.value.trim()||'',slug=$('#bookshelfAuthorSlug')?.value.trim().toLowerCase()||'',bookshelfLead=$('#bookshelfAuthorShelfLead')?.value.trim()||'';if(!displayName){setAuthorProfileStatus('作者名を入力してください。',true);return;}if(slug&&!/^[a-z0-9][a-z0-9_-]{2,29}$/.test(slug)){setAuthorProfileStatus('短い本棚IDは3〜30文字の半角英数字・ハイフン・アンダーバーで入力してください。',true);return;}setAuthorProfileStatus('作者情報を保存しています…');try{const payload=await authorRequest('/author-auth/profile',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({displayName,slug,bookshelfLead})});saveAuthorSession(authorToken,payload.author);setAuthorProfileStatus('公開作者情報を保存しました。');}catch(error){setAuthorProfileStatus(error.message||'作者情報を保存できませんでした。',true);}}
function headerPositionValue(id,fallback=50){const input=$(id),value=Number(input?.value);return Number.isFinite(value)?Math.max(0,Math.min(100,Math.round(value))):fallback;}
function renderAuthorHeaderPreviews(){const header=signedInAuthor?.bookshelfHeader,url=authorHeaderPublicUrl(header),preview=$('#authorHeaderDesktopPreview');if(!preview)return;preview.style.backgroundImage=url?`url("${url.replace(/"/g,'\\"')}")`:'';preview.style.backgroundPosition=`${headerPositionValue('#authorHeaderDesktopX',header?.desktopX??50)}% ${headerPositionValue('#authorHeaderDesktopY',header?.desktopY??50)}%`;const empty=preview.querySelector('em');if(empty)empty.hidden=!!url;}
function syncAuthorHeaderUI(){const header=signedInAuthor?.bookshelfHeader,controls=$('#authorHeaderPositionControls');if(!controls)return;controls.hidden=!header;for(const [id,key] of [['#authorHeaderDesktopX','desktopX'],['#authorHeaderDesktopY','desktopY']]){const input=$(id);if(input)input.value=String(header?.[key]??50);}renderAuthorHeaderPreviews();}
function setAuthorHeaderStatus(message,error=false){const node=$('#authorHeaderStatus');if(!node)return;node.textContent=message;node.classList.toggle('is-error',error);}
...
async function recoveredMasterFileFromServer(work){
  const workId=String(work?.workId||'').trim();
  if(!workId)throw new Error('WORK_ID_MISSING');
  const data=await authorRequest(`/author/works/${encodeURIComponent(workId)}/recover-master`,{method:'POST'});
  const doc=data?.scene;
  if(!doc||typeof doc!=='object')throw new Error('RECOVERED_MASTER_MISSING');
  const recoveredId=String(doc?.studio?.identity?.workId||'').trim();
  const ownerKey=String(doc?.studio?.identity?.ownerKey||'').trim();
  if(recoveredId!==workId||!/^[A-Za-z0-9_-]{32,128}$/.test(ownerKey))throw new Error('RECOVERED_MASTER_IDENTITY_INVALID');
  const manifest={package:'scene-package',packageVersion:'1.0',sceneFormat:'1.0',title:String(doc.title||work?.title||'Untitled'),author:String(doc.author||work?.author||''),language:String(doc.language||'und'),entry:'scene.json'};
  const blob=await makeStoreZip([
    {name:'scene.json',bytes:te.encode(JSON.stringify(doc,null,2))},
    {name:'manifest.json',bytes:te.encode(JSON.stringify(manifest,null,2))}
  ]);
  const safe=String(doc.title||work?.title||workId).replace(/[\\/:*?"<>|]+/g,'_');
  return new File([blob],`${safe}.scene`,{type:'application/zip'});
}
async function restoreServerPublishedMasters(){
  if(!authorToken||!Array.isArray(authorWorks)||!authorWorks.length)return 0;
  const localIds=new Set((await getAllWorks()).map(item=>String(item?.workId||'')).filter(Boolean));
  let restored=0;
  for(const work of authorWorks){
    const workId=String(work?.workId||'').trim();
    if(!workId||localIds.has(workId))continue;
    try{
      const file=await recoveredMasterFileFromServer(work);
      await addMaster(file,{silent:true});
      // Public hosted assets intentionally stay as HTTPS references in the
      // recovered Master. Studio can edit them immediately and a later normal
      // Master export re-embeds those assets through its existing packager.
      const row=await getWork(workId);
      if(row){
        const coverUrl=String(work?.coverUrl||'').trim();
        await putWork({...row,coverUrl:coverUrl||row.coverUrl||'',serverRecovered:true,publicationId:String(work?.publicationId||'')});
      }
      localIds.add(workId);restored++;
    }catch(error){console.warn('Published Master recovery skipped',workId,error);}
  }
  if(restored){
    const [masters,drafts]=await Promise.all([getAllWorks(),getAllStudioDraftMeta()]);
    const localMasters=await Promise.all(masters.map(async item=>hydrateStoredBookMetadata({...item,role:'master'})));
    const draftViews=drafts.filter(item=>!localMasters.some(master=>master.workId&&master.workId===item.workId)).map(item=>({...item,role:'studio-draft'}));
    shelfDataCache.created=[...localMasters,...draftViews];
...
function prepareShelfPlayerExit(dialog=null){
  if(dialog?.open)dialog.close();
  syncShelfScrollLock();
  saveShelfScroll(currentShelfTab,readShelfScrollY());
}
function leaveShelfForPlayer(href,dialog=null){
...
document.addEventListener('click',event=>{
  ...
  else if(target?.closest?.('.detail-author-shelf-link'))prepareShelfPlayerExit($('#detailDialog'));
...
function openDb(){return new Promise((resolve,reject)=>{const r=indexedDB.open(DB_NAME,DB_VERSION);r.onupgradeneeded=()=>{const db=r.result;if(!db.objectStoreNames.contains(WORKS))db.createObjectStore(WORKS,{keyPath:'workId'});if(!db.objectStoreNames.contains(READER_BOOKS))db.createObjectStore(READER_BOOKS,{keyPath:'copyId'});if(!db.objectStoreNames.contains(HANDOFF))db.createObjectStore(HANDOFF,{keyPath:'key'});if(!db.objectStoreNames.contains(WORK_RECOVERY))db.createObjectStore(WORK_RECOVERY,{keyPath:'recoveryId'});if(!db.objectStoreNames.contains(READER_RECOVERY))db.createObjectStore(READER_RECOVERY,{keyPath:'recoveryId'});};r.onsuccess=()=>resolve(r.result);r.onerror=()=>reject(r.error);});}
...
async function deleteWork(id){const db=await openDb();try{const t=db.transaction([WORKS,WORK_RECOVERY],'readwrite'),store=t.objectStore(WORKS),recovery=t.objectStore(WORK_RECOVERY),row=await request(store.get(id));if(row)recovery.put({recoveryId:`${id}:${Date.now()}`,workId:id,deletedAt:Date.now(),row});store.delete(id);await new Promise((resolve,reject)=>{t.oncomplete=resolve;t.onerror=()=>reject(t.error);t.onabort=()=>reject(t.error||new Error('Work delete aborted'));});}finally{db.close();}}
...
async function deleteReaderBook(id){const db=await openDb();try{const t=db.transaction([READER_BOOKS,READER_RECOVERY],'readwrite'),store=t.objectStore(READER_BOOKS),recovery=t.objectStore(READER_RECOVERY),row=await request(store.get(id));if(row)recovery.put({recoveryId:`${id}:${Date.now()}`,copyId:id,deletedAt:Date.now(),row});store.delete(id);await new Promise((resolve,reject)=>{t.oncomplete=resolve;t.onerror=()=>reject(t.error);t.onabort=()=>reject(t.error||new Error('Reader delete aborted'));});}finally{db.close();}}
...
async function inspectMaster(file){const entries=await readZipEntries(file);const sceneBytes=entries.get('scene.json');if(!sceneBytes)throw new Error('scene.json がありません。');const doc=parseJson(sceneBytes);const manifest=entries.get('manifest.json')?parseJson(entries.get('manifest.json')):{};const role=String(manifest.packageRole||doc?.package?.role||'').toLowerCase();if(role==='distribution')throw new Error('これは配布版です。本棚には作者の Master .scene を追加してください。');const workId=String(doc?.studio?.identity?.workId||doc?.workId||'').trim();if(!workId)throw new Error('Master の workId を確認できません。');if(!doc?.studio?.identity&&!entries.has('studio-state.json'))throw new Error('Master .scene と確認できないため追加を止めました。');let coverBlob=null;const coverPath=String(doc?.cover?.src||manifest?.cover?.image||'').replace(/^\.\//,'');if(coverPath&&entries.has(coverPath))coverBlob=new Blob([entries.get(coverPath)],{type:mime(coverPath)});return{workId,title:String(doc.title||manifest.title||'Untitled'),subtitle:String(doc?.metadata?.subtitle||doc?.subtitle||manifest?.subtitle||''),description:String(doc?.metadata?.description||doc?.description||manifest?.description||''),seriesId:String(doc?.metadata?.seriesId||manifest?.series?.id||''),seriesTitle:String(doc?.metadata?.seriesTitle||manifest?.series?.title||''),episode:String(doc?.metadata?.episode||manifest?.series?.episode||''),episodeNumber:Number(doc?.metadata?.episodeNumber||manifest?.series?.episodeNumber||0)||0,episodeTitle:String(doc?.metadata?.episodeTitle||manifest?.episodeTitle||''),author:String(doc.author||manifest.author||''),sceneCount:Array.isArray(doc.scenes)?doc.scenes.length:0,revision:Number(doc?.studio?.identity?.revision||0)||0,masterCreatedAt:String(doc?.studio?.identity?.createdAt||''),coverPresentation:{fontFamily:String(doc?.cover?.fontFamily||''),styles:doc?.cover?.styles||{},visibility:doc?.cover?.visibility||{}},coverBlob,coverUrl:/^https?:\/\//i.test(coverPath)?coverPath:'',blob:new Blob([await file.arrayBuffer()],{type:'application/octet-stream'})};}
...
async function inspectDistribution(file){
  const entries=await readZipEntries(file);const sceneBytes=entries.get('scene.json');
  if(!sceneBytes)throw new Error('scene.json がありません。');
  const doc=parseJson(sceneBytes);const manifest=entries.get('manifest.json')?parseJson(entries.get('manifest.json')):{};
  const role=String(manifest.packageRole||doc?.package?.role||'').toLowerCase();
  if(role!=='distribution')throw new Error('これは Distribution .scene ではありません。');
  const copyId=String(doc?.distribution?.copyId||manifest?.copyId||'').trim();
  const workId=String(doc?.distribution?.workId||manifest?.workId||doc?.workId||'').trim();
...
async function hydrateStoredBookMetadata(w){
  if(!w?.blob||w.shelfMetaVersion===5)return w;
  try{
    ...
    const next={...w,
      ...
      episodeNumber:Number(doc?.metadata?.episodeNumber||manifest?.series?.episodeNumber||w.episodeNumber||0)||0,
...
function applyDesktopColumns(value,{announce=false}={}){desktopShelfColumns=Math.min(6,Math.max(3,Number(value)||4));const grids=new Set([$('#grid'),localShelfViews.owned,localShelfViews.created].filter(Boolean));grids.forEach(grid=>grid.dataset.desktopColumns=String(desktopShelfColumns));try{localStorage.setItem(DESKTOP_COLUMNS_KEY,String(desktopShelfColumns));}catch(_){}const select=$('#desktopColumnsSelect');if(select)select.value=String(desktopShelfColumns);if(announce)toast(`PC ${desktopShelfColumns}列表示`);}
...
function readOwnedSeriesBoxes(){
  try{
    const value=JSON.parse(localStorage.getItem(OWNED_SERIES_BOXES_KEY)||'[]');
    if(!Array.isArray(value))return[];
    return value.map(box=>({
      seriesId:String(box?.seriesId||''),
      title:String(box?.title||'').trim(),
      copyIds:[...new Set((Array.isArray(box?.copyIds)?box.copyIds:[]).map(String).filter(Boolean))]
    })).filter(box=>box.seriesId&&box.title);
  }catch(_){return[];}
}
function writeOwnedSeriesBoxes(boxes){
  const normalized=(Array.isArray(boxes)?boxes:[]).map(box=>({seriesId:String(box?.seriesId||''),title:String(box?.title||'').trim(),copyIds:[...new Set((Array.isArray(box?.copyIds)?box.copyIds:[]).map(String).filter(Boolean))]})).filter(box=>box.seriesId&&box.title);
  try{localStorage.setItem(OWNED_SERIES_BOXES_KEY,JSON.stringify(normalized));ownedSeriesBoxes=normalized;return true;}
  catch(_){toast('この端末にBOXを保存できませんでした。');return false;}
}
function newOwnedSeriesId(){
  const uuid=globalThis.crypto?.randomUUID?.()||`${Date.now().toString(36)}_${Math.random().toString(36).slice(2)}`;
  return`owned_series_${uuid}`;
}
...
function publishedAuthorWorkForLocal(item,id){
  const workId=String(item?.workId||id||'').trim();
  return authorWorks.find(work=>String(work?.workId||'')===workId)||null;
}
async function setPublishedWorkState(item,id,makePublic){
  if(!authorToken)throw new Error('公開作品を操作するには作者ログインが必要です。');
  const remote=publishedAuthorWorkForLocal(item,id);
  const publicationId=String(remote?.publicationId||item?.publicationId||item?.publicId||'').trim();
  if(!publicationId)throw new Error('公開作品IDを確認できませんでした。');
  await authorRequest(`/work/${encodeURIComponent(publicationId)}/${makePublic?'republish':'unpublish'}`,{method:'POST'});
  if(remote)remote.state=makePublic?'public':'stopped';
...
async function permanentlyDeleteArchivedBook(tab,id){
  ...
  if(tab==='owned'){
    ...
    writeOwnedSeriesBoxes(ownedSeriesBoxes.map(box=>({...box,copyIds:box.copyIds.filter(copyId=>copyId!==id)})));
  }
  else if(tab==='created'){
    const item=(shelfDataCache.created||[]).find(book=>shelfIdOf(book)===id);
    const remote=publishedAuthorWorkForLocal(item,id);
    const publicationId=String(remote?.publicationId||item?.publicationId||item?.publicId||'').trim();
    if(publicationId){
      if(!authorToken)throw new Error('公開作品を完全削除するには作者ログインが必要です。');
      await authorRequest(`/work/${encodeURIComponent(publicationId)}`,{method:'DELETE'});
      authorWorks=authorWorks.filter(work=>String(work?.workId||'')!==String(item?.workId||id));
      authorSeries=authorSeries.map(series=>({...series,episodes:(series.episodes||[]).filter(ep=>String(ep?.workId||'')!==String(item?.workId||id))}));
    }
    if(item?.draftId)await deleteStudioDraft(item.draftId);
    if(item?.role==='master'||(item?.blob&&item?.workId))await deleteWork(item.workId||id);
  }
  writeIdList(SHELF_ARCHIVE_KEYS[tab],readIdList(SHELF_ARCHIVE_KEYS[tab]).filter(x=>x!==id));
  writeIdList(SHELF_ORDER_KEYS[tab],readIdList(SHELF_ORDER_KEYS[tab]).filter(x=>x!==id));
}
...
function renderReadLaterList(){
  ...
  list.innerHTML=items.length?items.map(item=>{
    ...
    return `<article class="read-later-item" data-read-later-id="${escapeHtml(item.publicationId)}"><a class="read-later-cover" href="${escapeHtml(detailHref||'#')}">${cover}</a><div class="read-later-copy"><h3>${escapeHtml(item.title||'Untitled')}</h3>${item.detail?`<p>${escapeHtml(item.detail)}</p>`:''}<small>${escapeHtml(item.authorName||'作者')}</small><div class="read-later-actions">${readHref?`<a data-read-later-read href="${escapeHtml(readHref)}">読む</a>`:''}<button type="button" data-read-later-remove="${escapeHtml(item.publicationId)}">外す</button></div></div></article>`;
  ...
  list.querySelectorAll('[data-read-later-read]').forEach(link=>link.onclick=event=>{event.preventDefault();leaveShelfForPlayer(link.href,$('#readLaterDialog'));});
}
...
function bindOfficialShelfInteractions(host=$('#officialBooks')){
  ...
  host.querySelectorAll('.official-book-cover[data-official-read]').forEach(cover=>{
    const open=()=>{const href=officialReadUrl(cover.dataset.officialRead);if(href)leaveShelfForPlayer(href);};
    cover.onclick=open;
...
function buildLocalShelfView(tab){
  ...
  const grouped=tab==='created'
    ?new Set(authorSeries.flatMap(series=>(series.episodes||[]).map(item=>item.workId)))
    :new Set(ownedSeriesBoxes.flatMap(series=>series.copyIds||[]));
  const archived=new Set(readIdList(SHELF_ARCHIVE_KEYS[tab]));
...
function seriesBoxesContentHtml(seriesShelf){
  const books=shelfDataCache[seriesShelf]||[];
  const local=new Map(books.map(book=>[shelfIdOf(book),book]));
  const source=seriesShelf==='owned'?ownedSeriesBoxes:authorSeries;
  const boxes=orderedSeriesBoxes(source.filter(series=>seriesBookIds(series,seriesShelf).some(id=>local.has(id))||seriesBookIds(series,seriesShelf).length===0),seriesShelf);
  return boxes.length?boxes.map(series=>{const boxBooks=seriesBookIds(series,seriesShelf).map(id=>local.get(id)).filter(Boolean),covers=boxBooks.slice(0,3),bookCount=seriesShelf==='owned'?boxBooks.length:(Number(series.episodeCount)||boxBooks.length);while(covers.length<3)covers.push(null);return`<button class="series-box" data-series-shelf="${seriesShelf}" data-series-id="${escapeHtml(series.seriesId)}" type="button"><span class="series-cover-stack">${covers.map((book,index)=>seriesStackCoverHtml(book,index)).join('')}</span><span class="series-box-copy"><small>SERIES BOX</small><strong>${escapeHtml(series.title)}</strong><em>${bookCount}冊</em></span></button>`;}).join(''):'<p class="series-box-empty">BOXを作ると、本をひとまとまりで並べられます。</p>';
}
function seriesShelfSnapshotHtml(seriesShelf){
  if(seriesShelf==='created'&&!authorToken)return'';
  return`<section class="series-shelf"><div class="series-shelf-head"><div><p class="eyebrow">SERIES BOX</p><h2>シリーズBOX</h2></div><button type="button">＋ BOXを作る</button></div><div class="series-boxes">${seriesBoxesContentHtml(seriesShelf)}</div></section>`;
}
function moveSeriesBoxBefore(sourceId,targetId,shelf=currentShelfTab){if(!sourceId||!targetId||sourceId===targetId)return;const key=seriesOrderKey(shelf),current=readIdList(key),sourceIndex=current.indexOf(sourceId),targetIndex=current.indexOf(targetId);if(sourceIndex<0||targetIndex<0)return;current.splice(targetIndex,0,current.splice(sourceIndex,1)[0]);writeIdList(key,current);const source=document.querySelector(`.series-box[data-series-id="${CSS.escape(sourceId)}"]`),target=document.querySelector(`.series-box[data-series-id="${CSS.escape(targetId)}"]`);if(source&&target)target.parentNode.insertBefore(source,sourceIndex<targetIndex?target.nextSibling:target);if(shelf==='created'&&authorToken)authorRequest('/author/series-order',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({seriesIds:current})}).catch(()=>toast('BOXの公開順を保存できませんでした。'));}
function syncSeriesShelf(){
  const shelf=$('#seriesShelf');if(!shelf)return;
  const seriesShelf=currentShelfTab==='owned'?'owned':'created';
  const visible=currentShelfTab==='owned'||(currentShelfTab==='created'&&!!authorToken);
  shelf.hidden=!visible;
  if(!visible){$('#seriesBoxes').innerHTML='';return;}
  $('#createSeriesBoxButton').textContent='＋ BOXを作る';
  $('#seriesBoxes').innerHTML=seriesBoxesContentHtml(seriesShelf);
  document.querySelectorAll('#seriesBoxes .series-box').forEach(button=>{
    button.draggable=true;
    button.onclick=event=>{if(suppressSeriesBoxClick){event.preventDefault();return;}openSeriesBoxEditor(button.dataset.seriesId||'',button.dataset.seriesShelf||seriesShelf);};
    button.ondragstart=event=>{if(draggingBookId)return;draggingSeriesBoxId=button.dataset.seriesId||'';suppressSeriesBoxClick=true;button.classList.add('is-box-dragging');event.dataTransfer?.setData('text/ahako-series-box',draggingSeriesBoxId);if(event.dataTransfer)event.dataTransfer.effectAllowed='move';};
    button.ondragover=event=>{if(draggingBookId){event.preventDefault();button.classList.add('is-drag-over');if(event.dataTransfer)event.dataTransfer.dropEffect='move';return;}if(draggingSeriesBoxId&&draggingSeriesBoxId!==button.dataset.seriesId){event.preventDefault();button.classList.add('is-box-drop-target');if(event.dataTransfer)event.dataTransfer.dropEffect='move';}};
    button.ondragleave=()=>button.classList.remove('is-drag-over','is-box-drop-target');
    button.ondrop=async event=>{event.preventDefault();event.stopPropagation();button.classList.remove('is-drag-over','is-box-drop-target');const targetShelf=button.dataset.seriesShelf||seriesShelf;if(draggingBookId){await addBookToSeries(targetShelf,button.dataset.seriesId,draggingBookId,button);return;}const sourceId=draggingSeriesBoxId||event.dataTransfer?.getData('text/ahako-series-box');if(sourceId)moveSeriesBoxBefore(sourceId,button.dataset.seriesId,targetShelf);};
    button.ondragend=()=>{draggingSeriesBoxId='';button.classList.remove('is-box-dragging');document.querySelectorAll('.series-box').forEach(box=>box.classList.remove('is-box-drop-target'));setTimeout(()=>{suppressSeriesBoxClick=false;},80);};
  });
}

async function addBookToSeries(shelf,seriesId,bookId,target){
  if(shelf==='owned'){
    const series=ownedSeriesBoxes.find(item=>item.seriesId===seriesId);
    if(!series||(shelfDataCache.owned||[]).every(item=>item.copyId!==bookId))return;
    await animateBookIntoBox(bookId,target);
    series.copyIds=[...series.copyIds.filter(id=>id!==bookId),bookId];
    writeOwnedSeriesBoxes(ownedSeriesBoxes);
    draggingBookId='';
    await render();
    toast(`「${series.title}」へ入れました。`);
    return;
  }
...
async function render({deferCoverRevoke=false,refreshOfficial=true}={}){
  ...
  for(const meta of await getAllStudioDraftMeta()){
    if(!meta?.draftId)continue;
    const old=meta.workId?mastersByWorkId.get(meta.workId):null;
    const draftView={...meta,workId:String(meta.workId||''),draftId:meta.draftId,role:'studio-draft',updatedAt:new Date(Number(meta.updatedAt)||Date.now()).toISOString()};
    if(old){
      const index=masters.indexOf(old);
      const merged={...old,...draftView,role:'master',blob:old.blob,coverBlob:old.coverBlob,draftId:meta.draftId,hasStudioDraft:true};
      masters[index]=merged;
...
function swipeShelfBodyHtml(tab){
  ...
  if(arranged.visible.length){
    ...
    if(cached){
      ...
      const series=seriesShelfSnapshotHtml(tab);
      return `${series||''}${snapshot.outerHTML}`;
    }
...
async function makeSelfContainedRecoveredMasterBlob(w){
  if(!w?.blob)return null;
  const entries=await readZipEntries(new File([w.blob],w.fileName||'master.scene',{type:'application/octet-stream'}));
  const sceneBytes=entries.get('scene.json');
  if(!sceneBytes)return w.blob;
  const doc=parseJson(sceneBytes);
  const assetUrls=new Set();
  const apiOrigin=new URL(API_BASE,location.href).origin;
  function collect(value){
  ...
  collect(doc);
  if(!assetUrls.size)return w.blob;
  const replacements=new Map(),packed=[];
  let n=0;
  for(const url of assetUrls){
    const res=await fetch(url,{cache:'no-store'});
    if(!res.ok)throw new Error(`ASSET_FETCH_${res.status}`);
    const bytes=new Uint8Array(await res.arrayBuffer());
    const u=new URL(url),base=(u.pathname.split('/').pop()||`asset_${++n}`).replace(/[^A-Za-z0-9._-]/g,'_');
    const name=`assets/${base}`;
    replacements.set(url,`./${name}`);packed.push({name,bytes});
  }
  function rewrite(value){
    if(typeof value==='string'){
      try{const u=new URL(value,location.href);return replacements.get(u.href)||value;}catch(_){return value;}
    ...
    if(Array.isArray(value))return value.map(rewrite);
    if(value&&typeof value==='object'){for(const k of Object.keys(value))value[k]=rewrite(value[k]);}
    return value;
  }
  rewrite(doc);
  const out=[];
  for(const [name,bytes] of entries){if(name!=='scene.json'&&!name.startsWith('assets/'))out.push({name,bytes});}
  out.push({name:'scene.json',bytes:te.encode(JSON.stringify(doc,null,2))},...packed);
  return makeStoreZip(out);
}
async function exportMasterBlob(w){
  if(w?.serverRecovered){
    toast('画像・音声をMasterへ格納しています…');
    const blob=await makeSelfContainedRecoveredMasterBlob(w);
    downloadBlob(blob,w.fileName||`${w.title||'master'}.scene`);
    toast('自己完結Masterを書き出しました。');
    return;
  }
  downloadBlob(w.blob,w.fileName||`${w.title||'master'}.scene`);
}
...
function detailIdentityHtml(w,isReader){
  ...
  const authorShelfUrl=isReader?readerBookAuthorShelfUrl(w):'';
  const authorName=escapeHtml(w.author||'作者未設定');
  const authorLine=authorShelfUrl?`<a class="detail-author-shelf-link detail-author-inline-link" href="${escapeHtml(authorShelfUrl)}"><strong>${authorName}</strong><span>作者の本棚へ</span><b aria-hidden="true">›</b></a>`:`<p class="detail-hero-author">${authorName}</p>`;
  return `<div class="detail-copy"><p class="eyebrow">${isReader?'MY COPY':'MASTER'}</p><h2>${escapeHtml(w.title)}</h2>${subtitle?`<p class="detail-hero-subtitle">${escapeHtml(subtitle)}</p>`:''}${authorLine}${episode?`<p class="detail-hero-episode">${escapeHtml(episode)}</p>`:''}${episodeTitle?`<p class="detail-hero-episode-title">${escapeHtml(episodeTitle)}</p>`:''}<p class="detail-hero-facts">${w.sceneCount||0} Scene${!isReader&&w.revision?` · revision ${w.revision}`:''}</p><p class="detail-hero-date">${stamp}</p></div>`;
}
...
async function editInStudio(w){if(w?.draftId){location.href=`../studio/?draft=${encodeURIComponent(w.draftId)}`;return;}await setHandoff(w.workId);location.href='../studio/?from=bookshelf';}
...
function installSceneDrop(){
  ...
  const internalDrag=e=>Boolean(draggingBookId||draggingSeriesBoxId||draggingSeriesWorkId||Array.from(e.dataTransfer?.types||[]).some(type=>String(type).startsWith('text/ahako-')));
  window.addEventListener('dragenter',e=>{if(internalDrag(e)||!hasFiles(e))return;e.preventDefault();depth++;document.body.classList.add('scene-drag-active');overlay?.setAttribute('aria-hidden','false');});
  window.addEventListener('dragover',e=>{if(internalDrag(e)||!hasFiles(e))return;e.preventDefault();if(e.dataTransfer)e.dataTransfer.dropEffect='copy';});
  window.addEventListener('dragleave',e=>{if(internalDrag(e)||!hasFiles(e))return;depth=Math.max(0,depth-1);if(depth===0){document.body.classList.remove('scene-drag-active');overlay?.setAttribute('aria-hidden','true');}});
  window.addEventListener('drop',async e=>{
    if(internalDrag(e)||!hasFiles(e))return;e.preventDefault();depth=0;document.body.classList.remove('scene-drag-active');overlay?.setAttribute('aria-hidden','true');
    const files=Array.from(e.dataTransfer?.files||[]).filter(f=>/\.(scene|zip)$/i.test(f.name||''));
    if(!files.length){AhakoDialog.alert('.scene ファイルをドロップしてください。');return;}
    let added=0;
    for(const file of files){try{await addDroppedScene(file);added++;}catch(err){console.error(err);AhakoDialog.alert(`${file.name||'ファイル'}\n${err.message||'本棚に追加できませんでした。'}`);}}
...
function openArchiveBox(){
  ...
  if(deleteSelected)deleteSelected.onclick=()=>{
    ...
    if(summary){
      ...
      const publishedCount=currentShelfTab==='created'?items.filter(entry=>{const item=(shelfDataCache.created||[]).find(book=>shelfIdOf(book)===entry.id);return !!publishedAuthorWorkForLocal(item,entry.id);}).length:0;summary.textContent=publishedCount?(items.length===1?`「${first}」をサーバーの公開作品も含めて完全に削除します。購入済みMY COPYは削除されません。`:`選択した${items.length}冊を完全に削除します。このうち公開中/公開停止中の${publishedCount}冊はサーバーからも削除します。購入済みMY COPYは削除されません。`):(items.length===1?`「${first}」を完全に削除します。`:`選択した${items.length}冊を完全に削除します。`);
    }
...
function setBookshelfAuthStatus(message='',error=false){const el=$('#bookshelfAuthorStatus');el.textContent=message;el.classList.toggle('is-error',error);}
function openBookshelfAuthor(){syncBookshelfAuthorUI();setBookshelfAuthStatus('');$('#bookshelfAuthorDialog').showModal();syncShelfScrollLock();}
async function requestBookshelfAuthorCode(){const email=$('#bookshelfAuthorEmail').value.trim(),displayName=$('#bookshelfAuthorDisplayName').value.trim();setBookshelfAuthStatus('送信しています…');try{await authorRequest('/author-auth/request-code',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email})});$('#bookshelfAuthorEmailForm').hidden=true;$('#bookshelfAuthorCodeForm').hidden=false;$('#bookshelfAuthorCode').focus();setBookshelfAuthStatus('メールに届いた6桁のコードを入力してください。');sessionStorage.setItem('ahako:author-login-email',email);sessionStorage.setItem('ahako:author-login-name',displayName);}catch(error){setBookshelfAuthStatus(error.message,true);}}
async function verifyBookshelfAuthorCode(){const email=sessionStorage.getItem('ahako:author-login-email')||$('#bookshelfAuthorEmail').value.trim(),displayName=sessionStorage.getItem('ahako:author-login-name')||$('#bookshelfAuthorDisplayName').value.trim(),code=$('#bookshelfAuthorCode').value.trim();setBookshelfAuthStatus('確認しています…');try{const payload=await authorRequest('/author-auth/verify-code',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email,displayName,code})});saveAuthorSession(payload.token,payload.author);await loadAuthorShelfData();$('#bookshelfAuthorDialog').close();toast('作者アカウントでログインしました。');}catch(error){setBookshelfAuthStatus(error.message,true);}}
function renderSeriesBookChoices(){const owned=editingSeriesShelf==='owned',published=new Set(authorWorks.map(work=>work.workId)),seriesSource=owned?ownedSeriesBoxes:authorSeries,occupiedByOther=new Set(seriesSource.filter(series=>series.seriesId!==editingSeriesId).flatMap(series=>seriesBookIds(series,editingSeriesShelf))),archived=new Set(readIdList(SHELF_ARCHIVE_KEYS[editingSeriesShelf])),local=(shelfDataCache[editingSeriesShelf]||[]).filter(book=>!archived.has(shelfIdOf(book))&&(owned||published.has(book.workId))&&(!occupiedByOther.has(shelfIdOf(book))||editingSeriesWorkIds.includes(shelfIdOf(book)))),byId=new Map(local.map(book=>[shelfIdOf(book),book])),selected=editingSeriesWorkIds.map(id=>byId.get(id)).filter(Boolean),available=local.filter(book=>!editingSeriesWorkIds.includes(shelfIdOf(book))&&!occupiedByOther.has(shelfIdOf(book))),ordered=[...selected,...available];$('#seriesBookChoices').innerHTML=ordered.length?ordered.map(book=>{const id=shelfIdOf(book),inBox=editingSeriesWorkIds.includes(id),position=editingSeriesWorkIds.indexOf(id);return`<div class="series-choice ${inBox?'is-selected':''}" draggable="${inBox}" data-work-id="${escapeHtml(id)}"><span class="series-choice-handle">${inBox?'☰':'○'}</span><span><strong>${escapeHtml(book.title)}</strong><small>${inBox?`${position+1}冊目・ドラッグで並び替え`:'このBOXへ追加できます'}</small></span>${inBox?'<span class="series-choice-actions"><button type="button" data-action="detail">詳細</button><button type="button" data-action="up" aria-label="上へ">↑</button><button type="button" data-action="down" aria-label="下へ">↓</button><button type="button" data-action="remove">外す</button></span>':'<button type="button" data-action="add">追加</button>'}</div>`;}).join(''):`<p class="series-choice-empty">${owned?'追加できる本はありません。':'追加できる公開作品はありません。'}</p>`;
  document.querySelectorAll('#seriesBookChoices .series-choice button').forEach(button=>button.onclick=()=>{const id=button.closest('.series-choice').dataset.workId,index=editingSeriesWorkIds.indexOf(id),action=button.dataset.action;if(action==='detail'){const selectedBook=byId.get(id);openDetail(owned?'distribution':(selectedBook?.role==='server-published'?'server-published':'master'),id);return;}if(action==='add'&&index<0)editingSeriesWorkIds.push(id);if(action==='remove'&&index>=0)editingSeriesWorkIds.splice(index,1);if(action==='up'&&index>0)[editingSeriesWorkIds[index-1],editingSeriesWorkIds[index]]=[editingSeriesWorkIds[index],editingSeriesWorkIds[index-1]];if(action==='down'&&index>=0&&index<editingSeriesWorkIds.length-1)[editingSeriesWorkIds[index+1],editingSeriesWorkIds[index]]=[editingSeriesWorkIds[index],editingSeriesWorkIds[index+1]];renderSeriesBookChoices();});
  let dragged='';document.querySelectorAll('#seriesBookChoices .series-choice.is-selected').forEach(row=>{row.ondragstart=e=>{dragged=row.dataset.workId;draggingSeriesWorkId=dragged;e.dataTransfer.setData('text/ahako-series-work',dragged);e.dataTransfer.effectAllowed='move';row.classList.add('is-dragging');};row.ondragend=()=>{dragged='';draggingSeriesWorkId='';document.querySelectorAll('#seriesBookChoices .series-choice').forEach(item=>item.classList.remove('is-dragging','is-drop-target'));};row.ondragover=e=>{if(!dragged||dragged===row.dataset.workId)return;e.preventDefault();e.dataTransfer.dropEffect='move';row.classList.add('is-drop-target');};row.ondragleave=()=>row.classList.remove('is-drop-target');row.ondrop=e=>{e.preventDefault();const source=dragged||e.dataTransfer.getData('text/ahako-series-work'),target=row.dataset.workId,from=editingSeriesWorkIds.indexOf(source),to=editingSeriesWorkIds.indexOf(target);if(from<0||to<0||from===to)return;editingSeriesWorkIds.splice(to,0,editingSeriesWorkIds.splice(from,1)[0]);dragged='';draggingSeriesWorkId='';renderSeriesBookChoices();};});
}
function openSeriesBoxEditor(seriesId='',shelf=currentShelfTab){editingSeriesShelf=shelf==='owned'?'owned':'created';if(editingSeriesShelf==='created'&&!authorToken){openBookshelfAuthor();return;}const source=editingSeriesShelf==='owned'?ownedSeriesBoxes:authorSeries,series=source.find(item=>item.seriesId===seriesId);editingSeriesId=series?.seriesId||'';editingSeriesWorkIds=series?seriesBookIds(series,editingSeriesShelf):[];$('#seriesBoxDialogTitle').textContent=series?'シリーズBOXを編集':'シリーズBOXを作る';$('#seriesBoxTitle').value=series?.title||'';$('#seriesBoxHelp').textContent=editingSeriesShelf==='owned'?'もっている本を選び、つまみで順番を入れ替えます。':'公開済みの本を選び、つまみで順番を入れ替えます。';$('#deleteSeriesBox').hidden=!series;$('#seriesBoxStatus').textContent='';$('#seriesBoxStatus').classList.remove('is-error');renderSeriesBookChoices();$('#seriesBoxDialog').showModal();syncShelfScrollLock();}
async function saveSeriesBox(){const title=$('#seriesBoxTitle').value.trim(),status=$('#seriesBoxStatus');if(!title){status.textContent='BOX名を入力してください。';return;}status.textContent='保存しています…';status.classList.remove('is-error');try{let id=editingSeriesId,isNew=!id;if(editingSeriesShelf==='owned'){if(!id)id=newOwnedSeriesId();const next=ownedSeriesBoxes.filter(box=>box.seriesId!==id);next.push({seriesId:id,title,copyIds:[...editingSeriesWorkIds]});if(!writeOwnedSeriesBoxes(next))throw new Error('この端末にBOXを保存できませんでした。');if(isNew)writeIdList(OWNED_SERIES_BOX_ORDER_KEY,[id,...readIdList(OWNED_SERIES_BOX_ORDER_KEY).filter(value=>value!==id)]);$('#seriesBoxDialog').close();await render();toast('シリーズBOXを保存しました。');return;}if(!id){const created=await authorRequest('/author/series',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({title})});id=created.series.seriesId;}else await authorRequest(`/author/series/${id}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({title})});await authorRequest(`/author/series/${id}/books`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({workIds:editingSeriesWorkIds})});if(isNew)writeIdList(SERIES_BOX_ORDER_KEY,[id,...readIdList(SERIES_BOX_ORDER_KEY).filter(value=>value!==id)]);await loadAuthorShelfData();$('#seriesBoxDialog').close();toast('シリーズBOXを保存しました。');}catch(error){status.textContent=error.message||'保存できませんでした。';status.classList.add('is-error');}}
async function deleteSeriesBox(){if(!editingSeriesId||!await AhakoDialog.confirm('BOXを削除しますか？ 本そのものは削除されません。'))return;try{if(editingSeriesShelf==='owned'){writeOwnedSeriesBoxes(ownedSeriesBoxes.filter(box=>box.seriesId!==editingSeriesId));writeIdList(OWNED_SERIES_BOX_ORDER_KEY,readIdList(OWNED_SERIES_BOX_ORDER_KEY).filter(id=>id!==editingSeriesId));$('#seriesBoxDialog').close();await render();toast('シリーズBOXを削除しました。');return;}await authorRequest(`/author/series/${editingSeriesId}`,{method:'DELETE'});writeIdList(SERIES_BOX_ORDER_KEY,readIdList(SERIES_BOX_ORDER_KEY).filter(id=>id!==editingSeriesId));await loadAuthorShelfData();$('#seriesBoxDialog').close();toast('シリーズBOXを削除しました。');}catch(error){$('#seriesBoxStatus').textContent=error.message;}}
...
$('#desktopColumnsSelect')?.addEventListener('change',event=>applyDesktopColumns(event.currentTarget.value,{announce:true}));
...
$('#bookshelfAuthorCodeForm').onsubmit=e=>{e.preventDefault();verifyBookshelfAuthorCode();};
$('#bookshelfAuthorResend').onclick=requestBookshelfAuthorCode;
$('#copyPublicAuthorShelf').onclick=async()=>{if(!signedInAuthor?.authorId)return;try{await navigator.clipboard.writeText(publicAuthorShelfShareUrl(signedInAuthor));toast('リンクカード対応の本棚URLをコピーしました。');}catch(_){toast('URLをコピーできませんでした。');}};
$('#saveBookshelfAuthorProfile')?.addEventListener('click',saveAuthorProfile);
$('#chooseAuthorHeader')?.addEventListener('click',()=>$('#authorHeaderInput')?.click());
$('#authorHeaderInput')?.addEventListener('change',async event=>{const file=event.currentTarget.files?.[0];try{await uploadAuthorHeader(file);}finally{event.currentTarget.value='';}});
for(const id of ['#authorHeaderDesktopX','#authorHeaderDesktopY'])$(id)?.addEventListener('input',()=>{renderAuthorHeaderPreviews();setAuthorHeaderStatus('プレビューを確認して「位置を保存」を押してください。');});
$('#saveAuthorHeaderPosition')?.addEventListener('click',saveAuthorHeaderPosition);
$('#deleteAuthorHeader')?.addEventListener('click',deleteAuthorHeader);
$('#bookshelfAuthorLogout').onclick=async()=>{const token=authorToken;saveAuthorSession('',null);authorSeries=[];authorWorks=[];if(token)fetch(`${API_BASE}/author-auth/logout`,{method:'POST',headers:{Authorization:`Bearer ${token}`}}).catch(()=>{});$('#bookshelfAuthorDialog').close();await render();};
$('#createSeriesBoxButton').onclick=()=>openSeriesBoxEditor('');
$('#closeSeriesBox').onclick=()=>$('#seriesBoxDialog').close();
$('#saveSeriesBox').onclick=saveSeriesBox;
$('#deleteSeriesBox').onclick=deleteSeriesBox;
...
$('#distributionInput').onchange=async e=>{const file=e.target.files?.[0];if(!file)return;try{const expected=e.target.dataset.replace||'';const id=await addDistribution(file);if(expected&&expected!==id)toast('別の一冊だったため、新しい一冊として追加しました。');applyShelfTab('owned');$('#detailDialog').close();await render();}catch(err){AhakoDialog.alert(err.message||'Distributionを追加できませんでした。');}finally{e.target.value='';e.target.dataset.replace='';}};
$('#fileInput').onchange=async e=>{const file=e.target.files?.[0];if(!file)return;try{const expected=e.target.dataset.replace||'';const id=await addMaster(file);if(expected&&expected!==id)toast('別作品のMasterだったため、その作品として追加しました。');setCreatedTabVisible(true);applyShelfTab('created');$('#detailDialog').close();await render();}catch(err){AhakoDialog.alert(err.message||'Masterを追加できませんでした。');}finally{e.target.value='';e.target.dataset.replace='';}};
$('#backupButton').onclick=()=>{closeBookshelfMenu();backupShelf().catch(e=>AhakoDialog.alert(e.message));};
$('#restoreButton').onclick=()=>{closeBookshelfMenu();$('#restoreInput').click();};
$('#restoreInput').onchange=async e=>{const f=e.target.files?.[0];if(!f)return;try{await restoreShelf(f);}catch(err){AhakoDialog.alert(err.message||'復元できませんでした。');}finally{e.target.value='';}};
$('#exportBoxButton')?.addEventListener('click',()=>exportStorageBox().catch(e=>AhakoDialog.alert(e.message||'箱を書き出せませんでした。')));
$('#importBoxButton')?.addEventListener('click',()=>$('#boxImportInput')?.click());
$('#boxImportInput')?.addEventListener('change',async e=>{const f=e.target.files?.[0];if(!f)return;try{await importStorageBox(f);}catch(err){AhakoDialog.alert(err.message||'箱を読み込めませんでした。');}finally{e.target.value='';}});
