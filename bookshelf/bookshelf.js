...
function syncAuthorProfileUI(){const name=$('#bookshelfAuthorProfileName'),slug=$('#bookshelfAuthorSlug'),lead=$('#bookshelfAuthorShelfLead'),url=$('#bookshelfAuthorShortUrl');if(name)name.value=signedInAuthor?.displayName||'';if(slug)slug.value=signedInAuthor?.slug||'';if(lead)lead.value=signedInAuthor?.bookshelfLead||'';if(url)url.textContent=signedInAuthor?.slug?publicAuthorShelfUrl(signedInAuthor):'未設定の場合は従来の本棚URLを使用します。';}
...
$('#copyPublicAuthorShelf').onclick=async()=>{if(!signedInAuthor?.authorId)return;try{await navigator.clipboard.writeText(publicAuthorShelfUrl(signedInAuthor));toast('公開本棚のURLをコピーしました。');}catch(_){toast('URLをコピーできませんでした。');}};
