(() => {
  ...
  let localSourceKey = 'local:scene';
  const source = () => LOCAL_MODE ? localSourceKey : (requested || DEFAULT_SCENE);
  ...
  async function loadDocument(doc, options = {}) {
    ...
    if (!doc || typeof doc !== 'object') throw new Error('scene.json を読み込めませんでした。');
    if (LOCAL_MODE) localSourceKey = String(options.sourceKey || 'local:scene');
    const hydrated = LOCAL_MODE ? doc : hydratePublicAudio(doc);
    ScenePlayerCore.validate(hydrated);
  ...
  async function fetchScene() {
    ...
    return loadDocument(doc, { sourceKey: src });
  }
  ...
  retryButton.addEventListener('click', () => {
    if (LOCAL_MODE && typeof window.SceneLocalLoader?.openPicker === 'function') {
      window.SceneLocalLoader.openPicker();
      return;
    }
  ...
  function unloadDocument() {
    ...
    documentData = null;
    documentSourceKey = '';
    resetResonanceSession(0);
  }

  window.ScenePublicPlayer = {
    version: '0.3.23-ios-stable-audio-elements-local-v3',
    ...
    loadDocument(doc, options = {}){ return loadDocument(doc, options); },
    showIntro(){ return showIntro(); },
    unloadDocument(){ return unloadDocument(); }
  };

  if (!LOCAL_MODE) fetchScene().catch(showError);
  bindReportControls();
  if (LOCAL_MODE) setReportVisible(false);
})();
