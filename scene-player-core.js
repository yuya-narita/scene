  ...
  class ScenePlayerCore {
    ...
    _measureScenePositions(nodes, sceneEntries, extraGap = 0) {
      ...
      metrics.forEach((item, i) => {
        const balloon = item.scene?.presentation?.balloon;
        if (balloon?.positioned) positions[i] = stageHeight * clamp(asNumber(balloon.y, 48), 5, 95) / 100 - item.height / 2;
      });
    ...
    _sceneNode(scene, active, age) {
      ...
      if (hasMangaBalloon && balloonConfig.positioned) {
        ...
        article.dataset.balloonPositioned = 'true';
        article.style.width = `${width}%`;
        article.style.left = `${x - width / 2}%`;
      }
    ...
    _buildDisappearDust(article, fade) {
      ...
      // Narrow text slices vanish from right to left. Only the moving erosion
      // edge emits particles, which makes the word weather away instead of break.
      const strips = 12;
