      ...
      const draw=()=>{
        ...
        if(text&&frame.dataset.writingMode==='vertical-rl'&&!frame.dataset.verticalInkFitted){
          ...
          try{
            ...
            range.detach?.();
            if(rects.length)inkWidth=Math.max(...rects.map(item=>item.right))-Math.min(...rects.map(item=>item.left));
          }catch(_){}
          const fittedWidth=Math.ceil(Math.max(text.scrollWidth,inkWidth,text.getBoundingClientRect().width)+2);
          text.style.width=`${fittedWidth}px`;
          frame.dataset.verticalInkFitted='true';
        }
